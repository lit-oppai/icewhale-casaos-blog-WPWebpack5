...
(function($, window, document, undefined) {
	$.fn.formchimp = function(settings) {
		...
		var defaults = {
			...
			'onMailChimpSuccess': function(msg) {},	// Callback that fires on success.
			'onMailChimpError': function(msg) {},		// Callback that fires on errors.
		...
		$($form).on('submit', function(event) {
			...
			$.ajax({
				...
				if (data.result === 'success') {
					...
					// Run callback
					var params = { msg:responseMessage};
					defaults.onMailChimpSuccess.call(params);
					...
					var params = { msg:responseMessage};
					defaults.onMailChimpError.call(params);
				}
